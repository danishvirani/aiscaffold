...
> **Update (2026-06-04): multi-language.** Per [ADR-002](decisions/ADR-002-multi-language.md), aiscaffold now targets multiple languages via `--lang`, superseding the stdlib-only-Python wedge of ADR-001. Architecture: `templates/_shared/` (language-agnostic plugin + skill + LICENSE) merged with `templates/<lang>/` per-language trees. **Python, Go, and Rust are complete and verified end-to-end** (each builds/lints/tests + a live MCP stdio roundtrip; wheel packaging verified incl. dot-dir files). Rust uses `serde_json` for its MCP server (no MCP SDK), per [ADR-002](decisions/ADR-002-multi-language.md); the dep is gated behind `--no-mcp` (zero deps without the server). **Node is the next language tree.** `--lang` advertises only languages with a complete tree.
