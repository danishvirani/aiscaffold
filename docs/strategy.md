...
This is the "why aiscaffold exists, why this shape" doc. Build sequence is in [roadmap.md](roadmap.md). The single biggest engineering decision is in [decisions/ADR-001-stdlib-only.md](decisions/ADR-001-stdlib-only.md).
...
**aiscaffold is a one-shot scaffolder that emits all four surfaces, already wired, stdlib-only, curl-installable. One command, 30 seconds, a distributable AI tool.**
...
- The demo is **shorter than the official path**. `pipx run aiscaffold my-tool` finishes in ~30s with a publishable tool. Anthropic's `/plugin-dev:create-plugin` is a multi-minute interactive chat.
...
- **The scaffolder embodies what it promotes.** aiscaffold is stdlib-only, so its output is stdlib-only. A generator that drags in deps while preaching zero-deps is a lie.
...
**DoD:** `pipx run aiscaffold my-tool` produces a working CLI + Claude Code plugin + MCP server + skill + brief + install.sh + CI, wired up. 60 seconds from invocation to "Claude can use my tool."

**Architecture:**
- Templates live in `src/aiscaffold/templates/` as plain files.
...
**Final v0.1 acceptance test:** `pipx run aiscaffold redink` should produce output matching redink's v0.0 scaffold shape. If it does, the scaffolder is correct. (This means matching the shape — not rewriting redink, which is built by other sessions.)
